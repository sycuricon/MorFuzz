...
extern "C" void cosim_init (const char *testcase, unsigned char verbose) {
  cfg.elffiles = std::vector<std::string> {testcase, "../../../debug_rom/debug_rom.elf"};
  cfg.isa = spike_misa;
  cfg.blind = false;
  cfg.commit_ecall = false;
  cfg.sync_state = false;
  if (verbose)
    cfg.verbose = true;
  simulator = new cosim_cj_t(cfg);
  simulator->setup_debug_mode();
  return;
}
...
extern "C" int coverage_collector(unsigned long int cov) {
  ...
  sprintf(current_log, "[CJ] (%d/%d/%d) coverage summary %d (+%d),\tinstruction count %ld (+%ld),\tpassed time %d hours, %d minutes, %d seconds\texception count %d\n", 
    elf_current, round_current, left_round, cov, delta_cov, inst_cnt_summary + inst_cnt, inst_cnt - inst_cnt_current, passed_time / 3600, (passed_time%3600) / 60, passed_time % 60, exception_counter);
  printf(current_log);
  ...
  if (tohost == 3 && left_round > 0) {
    ...
    sprintf(cmd, "%s %s --input=%016lx%016lx", pygen_bin, razzle_opt, shuffle_seed, seed);
    system(cmd);
